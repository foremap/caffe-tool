...
def find_parameters(dataset_pathname, options=''):
	...
	if best_g != None:
		...
		best_cg += [2.0**best_g]
	print('{0} {1}'.format(' '.join(map(str,best_cg)), best_rate)) #print the best result
	#output the best result in filename.out
	cg_file = open('cg.txt', 'w')
	output_str = ('{0} {1}'.format(' '.join(map(str,best_cg)), best_rate))
	cg_file.write(output_str)
	cg_file.flush()
	cg_file.close()
